...
parser.add_argument('--defense_mode', type=str, default="prune",
                    choices=['prune', 'isolate', 'none'], help="Mode of defense")
...
parser.add_argument('--test_model', type=str, default='GCN',
                    choices=['GCN', 'GAT', 'GraphSage', 'GIN'], help='Model used to attack')
parser.add_argument('--evaluate_mode', type=str, default='1by1',
                    choices=['overall', '1by1'], help='Model used to attack')
...
if args.dataset == 'ogbn-arxiv':  # 不知道为啥要自己构造 val_mask 和 test_mask
    nNode = data.x.shape[0]
    setattr(data, 'train_mask', torch.zeros(nNode, dtype=torch.bool).to(device))  # data.train_mask = torch.zeros
...
# 非测试集非验证集部分的节点 idx  todo:加上了valmask不知道正确性
